...
inline size_t __hash_str(const char *key) {
  size_t hash = MAGIC;
  int c;
  while ((c = *key++)) {
...
inline size_t __hash_u64(u64 key) {
  size_t hash = MAGIC;
  int c;
  while ((c = (key & 0xff))) {
    hash = ((hash << 5) + hash) + c; // hash * 33 + c
...
#define ht_get(ht, __k)                                                        \
  _Generic((__k), char *: __ht_get_str, default: __ht_get)(                    \
      (ht), (__k), offsetof(typeof(*(ht)->nodes[0]), key),                   \
      offsetof(typeof(*(ht)), nodes), sizeof(*(ht)->nodes),                    \
      offsetof(typeof(*(ht)->nodes[0]), value),                                \
      offsetof(typeof(*(ht)->nodes[0]), next));

static inline void *__ht_get_str(void *ht, size_t hash, size_t nodes_offset,
                                 size_t node_size, size_t value_offset) {
  return NULL;
}
