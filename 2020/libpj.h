...
#define ma_inbounds(ma, x, y) ((0 <= (x) && (x) <= (ma)->nx) && (0 <= (y) && (y) <= (ma)->ny))
...
#define sb_from_cstr(__cstr) (String_Builder){.items = strdup(__cstr), .count = strlen(__cstr), .capacity = strlen(__cstr)}
